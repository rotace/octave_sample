    ...
    methods
        ...
        function idx = search_target_index(self, state, trajectory)
            ...
            if isnan(self.old_nearest_point_index)
                ...
                % dy = state.rear_y .- cy;
                % d = sqrt( dx.^2 .+ dy.^2 );
                d = state.calc_distance(trajectory.cx, trajectory.cy, true);
                [~, idx] = min(d);
                self.old_nearest_point_index = idx;
            else
                idx = self.old_nearest_point_index;
                distance_this = state.calc_distance(trajectory.cx(idx), trajectory.cy(idx), true);
                
                while true
                    distance_next = state.calc_distance(trajectory.cx(idx+1), trajectory.cy(idx+1), true);
                    if distance_this < distance_next
                        break;
                    end
                    if idx + 1 < length(trajectory.cx)
                        idx += 1;
                    end
            ...
            while Lf > state.calc_distance(trajectory.cx(idx), trajectory.cy(idx), true)
                if idx + 1 >= length(trajectory.cx)
                    break;
                end
                idx+= 1;
